...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      // Extreme downscaling for performance (renders at 25% resolution)
      const dpr = 0.25; 
    ...
    const onMouseMove = (e) => {
      // Removed getBoundingClientRect() completely to fix layout thrashing stutter
    ...
    const animate = (t) => {
      ...
      // Increased interpolation speed from 0.08 to 0.2 for snappier mouse follow
      smoothMouseRef.current[0] += (mouseRef.current[0] - smoothMouseRef.current[0]) * 0.2;
      smoothMouseRef.current[1] += (mouseRef.current[1] - smoothMouseRef.current[1]) * 0.2;
      hoverAmountRef.current += ((isHoveredRef.current ? 1 : 0) - hoverAmountRef.current) * 0.15;
